...
static json_value_t * getJSONValue(tvbuff_t *skylink_tvb, guint offset, const gchar *str_match, gint str_length, gboolean string)
{
  ...
  if (word_offset > 0)
  {
    ...
    return_val->value = (gchar *)tvb_get_string(skylink_tvb, val_offset, val_length);
...
void proto_register_skylink(void)
{
  ...
  register_dissector("skylink", dissect_skylink, proto_skylink);
}
...
void proto_reg_handoff_skylink(void)
{
  ...
  if (!initialized) {
    skylink_handle = create_dissector_handle(dissect_skylink, proto_skylink);
